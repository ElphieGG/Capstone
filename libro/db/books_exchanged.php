...
// Query to fetch books where book_status is "Exchanged"
$sql = "SELECT b.id, b.title, b.description, b.category, b.book_condition, b.book_status, b.image, 
        ...
        WHERE b.book_status = 'Exchanged'";  // Filter for only Exchanged books
...
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {
        ...
        // Display "Posted by: FirstName LastName" instead of ID
        ...
        echo "<td>";
     //   echo "<a class='btn btn-success btn-sm' href='edit_books.php?id=" . $row['id'] . "'><i class='fas fa-edit'></i> Edit</a> ";
    //    echo "<a class='btn btn-danger btn-sm' href='delete_bookexchanged_admin.php?id=" . $row['id'] . "' onclick='return confirm(\"Are you sure you want to delete this book?\")'><i class='fas fa-trash-alt'></i> Delete</a>";
        echo "</td>";
        echo "</tr>";
    }
} else {
    echo "<tr><td colspan='8' class='text-center'>No exchanged books found</td></tr>";
}
...
function printBooksReport() {
    ...
    document.body.innerHTML = brandLogo + "<h2>Books Exchanged Report</h2>" + printContent;
    window.print();
