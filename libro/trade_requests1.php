...
// Query to get trade requests with user details
$sql = "SELECT tr.id, u.first_name, u.last_name, IFNULL(b.title, 'Unknown Book') AS book_title, tr.created_at, tr.status
        FROM trade_requests tr
        JOIN tbluser u ON tr.user_id_from = u.user_id
        LEFT JOIN tblbook b ON tr.book_id_to = b.id
        WHERE tr.user_id_to = ?
...
<div class="container mt-3">
  <p>Welcome, <?php echo $_SESSION['username']; ?>!</p>
    ...
    <h2 class="mb-3">Trade Requests</h2>-->

    <?php if ($result->num_rows > 0): ?>
        <?php while ($request = $result->fetch_assoc()): ?>
            <div class="card mb-3 shadow-sm">
                <div class="card-body">
                    <h5 class="card-title">Trade Request</h5>
                    <p class="card-text"><strong>Requested by:</strong> <?= htmlspecialchars($request['first_name'] . " " . $request['last_name']) ?></p>
                    <p class="card-text"><strong>Book Requested:</strong> <?= htmlspecialchars($request['book_title']) ?></p>
                    <p class="card-text"><strong>Request Date:</strong> <?= date("F j, Y, g:i a", strtotime($request['created_at'])) ?></p> <!-- Formatted timestamp -->

                    <?php if ($request['status'] == 'pending'): ?>
                        <a href="approve_trade.php?request_id=<?= $request['id'] ?>" class="btn btn-success me-2">✅ Accept</a>
                        <a href="reject_trade.php?request_id=<?= $request['id'] ?>" class="btn btn-danger">❌ Decline</a>
        ...
        <?php endwhile; ?>
    <?php else: ?>
        <p class="text-muted">No trade requests at the moment.</p>
    <?php endif; ?>
