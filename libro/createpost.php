...
//                 $biddingEndTime = $_POST['bidding_end_time'] ?? null;
// $buyOutPrice = $_POST['buy-out-price'] ?? null; // Optional
...
if ($book_status === "For Bidding") {
    ...
    // ✅ Directly assign the selected date from form
    $bidding_end_time_str = $biddingEndTime;
} elseif ($book_status === "For Sale") {
    $bidding_start_price = $_POST['bidding-price']; // Store Price in bidding_start_price
    $buyOutPrice = null; // ✅ Good practice to set it
    $bidding_end_time_str = null; // ✅ Good practice to clear it
    ...
    <div class="form-container">
        ...
        <div class="form-right">
            ...
            <div id="bidding-price-container">
                ...
                <input type="number" id="bidding-price" name="bidding-price" step="0.01" required>
            </div>
            ...
            <div id="bidding-duration-container">
                ...
                <input type="number" id="bidding-duration" name="bidding-duration" step="1" min="1" required> -->
                <label for="bidding-end-time">Bidding End Time:</label>
<input type="datetime-local" id="bidding-end-time" name="bidding_end_time">
            </div>

            <label for="meeting-spot">Meeting Spot:</label>
            <!-- <input type="text" id="meeting-spot" name="meeting-spot" required> --->
            <select id="meeting-spot" name="meeting-spot" required>
...
function toggleBiddingFields() {
    ...
    var biddingPriceContainer = document.getElementById("bidding-price-container");
    var biddingDurationContainer = document.getElementById("bidding-duration-container");

    if (status === "For Exchange") {
        biddingPriceContainer.style.display = "none";
        biddingDurationContainer.style.display = "none";
    } else if (status === "For Sale") {
        biddingPriceContainer.style.display = "block";
        biddingDurationContainer.style.display = "none";
        priceLabel.innerText = "Price:";
    } else if (status === "For Bidding") {
        biddingPriceContainer.style.display = "block";
        biddingDurationContainer.style.display = "block";
        priceLabel.innerText = "Bidding Start Price:";
    }
}

document.addEventListener("DOMContentLoaded", function() {
    toggleBiddingFields();
    document.getElementById("book-status").addEventListener("change", toggleBiddingFields);
});
</script>
</body>
</html>
