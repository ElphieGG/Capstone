...
<?php
include 'config.php'; // Ensure your DB connection file is here

if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && isset($_POST['detail_id'])) {
...
<div class="products-box">
    ...
    <div class="book-container">
        ...
        <th>Price</th>
        <th>Order Date</th>
        <th>Order Status</th><th>Action</th>
                    <th>Book Rating</th>
                    <th>Book Review</th>
                    <th>Seller Rating</th>
                    <th>Seller Review</th>
...
<td><?= isset($sale['user_id']) ? htmlspecialchars($sale['user_id']) : 'N/A' ?></td>
        <td><?= $sale['price'] ?></td>
        <td><?= $sale['order_date'] ?></td>
<td><?= isset($sale['order_status']) ? htmlspecialchars($sale['order_status']) : 'Pending' ?></td>

<td>
    <form method="POST" style="display:inline;">
        <input type="hidden" name="detail_id" value="<?= $sale['detail_id'] ?>">
        <input type="hidden" name="action" value="approve">
        <button type="submit" style="background-color: green; color: white; padding: 5px;">Approve</button>
    </form>
    <form method="POST" style="display:inline;">
        <input type="hidden" name="detail_id" value="<?= $sale['detail_id'] ?>">
        <input type="hidden" name="action" value="decline">
        <button type="submit" style="background-color: red; color: white; padding: 5px;">Decline</button>
    </form>
</td>
