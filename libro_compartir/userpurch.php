...
<div class="products-box">
    ...
    <div class="book-container">
        <?php if (empty($purchases)): ?>
            <p style="text-align: center;">You have not made any purchases yet.</p>
        <?php else: ?>
            <?php foreach ($purchases as $purchase): ?>
                <div class="book-item">
                    <img src="data:image/jpeg;base64,<?php echo base64_encode($purchase['image']); ?>" class="book" alt="Purchase">
                    <p><?php echo htmlspecialchars($purchase['title']); ?></p>
                    <?php if ($purchase['rated'] == 0): ?>
                        <button class="btn-action" onclick="openModal('<?php echo $purchase['title']; ?>', '<?php echo $purchase['book_id']; ?>')">Rate</button>
                    <?php else: ?>
                        <button class="btn-action" disabled>Rated</button>
                    <?php endif; ?>
                </div>
            <?php endforeach; ?>
        <?php endif; ?>
...
<div id="rateModal" class="modal">
    <div class="modal-content">
        ...
        <h2>Rate the Book and Seller</h2>
        <p id="bookTitle"></p>
        <input type="number" id="bookRating" class="rating-input" min="1" max="5" placeholder="Book Rating (1-5)">
        <textarea id="bookReview" class="rating-input" placeholder="Write a book review"></textarea>
        
        <input type="number" id="sellerRating" class="rating-input" min="1" max="5" placeholder="Seller Rating (1-5)">
        <textarea id="sellerReview" class="rating-input" placeholder="Write a seller review"></textarea>
...
<script>
function openModal(title, bookId) {
    document.getElementById('bookTitle').innerText = title;
    document.getElementById('rateModal').setAttribute('data-book-id', bookId);
...
function submitRating() {
    let bookId = document.getElementById('rateModal').getAttribute('data-book-id');
    let bookTitle = document.getElementById('bookTitle').innerText;
    let bookRating = document.getElementById('bookRating').value;
    let bookReview = document.getElementById('bookReview').value;
    let sellerRating = document.getElementById('sellerRating').value;
    let sellerReview = document.getElementById('sellerReview').value;

    if (!bookRating || !sellerRating) {
        Swal.fire({
            icon: 'warning',
            title: 'Missing Ratings!',
            text: 'Please provide ratings for both the book and the seller.',
    ...
    formData.append("sellerReview", sellerReview);
    ...
    .then(data => {
        if (data.trim() === "success") {
            ...
            Swal.fire({
                icon: 'success',
                title: 'Thank You!',
                text: 'Your review and ratings have been submitted successfully!',
                confirmButtonColor: '#3085d6'
            }).then(() => {
                location.reload(); // Reload the page after success alert
            });
        } else {
            Swal.fire({
                icon: 'error',
                title: 'Submission Failed!',
                text: 'Error submitting rating: ' + data,
