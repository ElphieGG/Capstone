...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'], $_POST['detail_id'])) {
    ...
    if ($stmt) {
        $stmt->bind_param("si", $status, $detailId);
        if ($stmt->execute()) {
            echo "<script>alert('Status updated successfully.');</script>";
            echo "<pre>Detail ID: $detailId | Status: $status | Affected Rows: " . $stmt->affected_rows . "</pre>";
            echo "<script>alert('Status updated successfully.');</script>";
        } else {
...
<div class="products-box">
    ...
    <div class="book-container">
        <table class="sales-table">
            <thead>
                <tr>
<td><strong>Detail ID:</strong> <?= $row['detail_id'] ?? 'NOT SET' ?></td>
                    <th>Image</th>
                    <th>Title</th>
                    <th>Buyer Name</th>
        <th>Price</th>
...
<td>
    ...
    <input type="hidden" name="detail_id" value="<?= $row['detail_id'] ?>">
        <button type="submit" name="action" value="approve" style="background-color: green; color: white; border: none; padding: 5px 10px;">Approve</button>
        <button type="submit" name="action" value="decline" style="background-color: red; color: white; border: none; padding: 5px 10px;">Decline</button>
    </form>
</td>
