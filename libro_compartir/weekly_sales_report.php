...
// SQL Query to fetch weekly sales report
// Get month and year from user input (e.g., via GET request)
// Get month and year from user input (e.g., via GET request)
$month = isset($_GET['month']) ? (int)$_GET['month'] : date('m');
$year = isset($_GET['year']) ? (int)$_GET['year'] : date('Y');

// SQL Query to fetch weekly sales report for a specific month and year
$sql = "
SELECT 
    WEEK(o.order_date, 1) - WEEK(DATE_SUB(o.order_date, INTERVAL DAY(o.order_date)-1 DAY), 1) + 1 AS week_number,
...
ORDER BY year_number DESC, month_number DESC, week_number ASC;";
...
// Function to format week number
function format_week($week_number) {
    $week_labels = ["1st Week", "2nd Week", "3rd Week", "4th Week", "5th Week"];
    return $week_labels[$week_number - 1] ?? "Week $week_number";
}
...
if ($result->num_rows > 0) {
    ...
    while ($row = $result->fetch_assoc()) {
        echo "<tr>
                ...
                <td>" . format_week($row["week_number"]) . "</td>
    ...
    echo "No records found for the selected month and year.";
}
